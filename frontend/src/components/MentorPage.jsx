...
export default function MentorPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        .from("users")
        .select(
          "id, name, year, bio, skills, interests, ai_interest, experience_years, preferred_meeting, points, profile_picture"
        )
        .eq("id", userId)
        .single();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        .from("interactions")
        .select("to_user, weight")
        .eq("type", "like")
        .eq("from_user", userId)
        .in("to_user", mentorIds);
      if (!error) {
        const map = {};
  ...
  const toggleLike = async (mentorId) => {
    ...
    if (isLiked) {
      ...
      const { error } = await supabase
        .from("interactions")
        .insert({ from_user: userId, to_user: mentorId, type: "like", weight: 1 });
      if (!error) {
      ...
      <div className="bg-gradient-to-b from-blue-100 via-blue-200 to-blue-300 text-gray-900 min-h-screen">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 m-10">
          {topMentors.map(({ mentor, score }) => (
            <div
              key={mentor.id}
              className="hover-pulse transition transform hover:scale-105 hover:ring-4 hover:ring-indigo-300 hover:ring-opacity-40 hover:ring-offset-2 bg-white shadow-lg rounded-xl p-6 flex flex-col items-center text-center border border-blue-200"
            >
              ...
              <div className="text-sm text-gray-600 mb-2">
                {mentor.year || "N/A"} • {mentor.experience_years || 0} yrs
                experience
              </div>
              ...
              <div className="flex gap-2">
                <a
                  ...
                  className={`px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-full text-sm shadow-sm ${
